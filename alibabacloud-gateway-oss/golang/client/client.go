...
type Client struct {
  ...
  Default_additional_headers  []*string
}
...
func (client *Client)Init()(_err error) {
  ...
  client.Default_additional_headers = []*string{tea.String("range"), tea.String("if-modified-since")}
  return nil
}
...
func (client *Client) GetAuthorization (signatureVersion *string, bucketName *string, pathname *string, method *string, query map[string]*string, headers map[string]*string, ak *string, secret *string, regionId *string) (_result *string, _err error) {
  ...
  if !tea.BoolValue(util.IsUnset(signatureVersion)) {
    ...
    if tea.BoolValue(string_.Equals(signatureVersion, tea.String("v2"))) {
      additionalHeaderNames, _err := client.GetAdditionalHeaderNamesV2(headers)
      if _err != nil {
        return _result, _err
      }

      sign, _err = client.GetSignatureV2(bucketName, pathname, method, query, headers, secret, additionalHeaderNames)
      if _err != nil {
        return _result, _err
      }

      additionalHeaders, _err := client.JoinSemicolon(additionalHeaderNames)
      if _err != nil {
        return _result, _err
      }

      if tea.BoolValue(util.Empty(additionalHeaders)) {
        _result = tea.String("OSS2 AccessKeyId:" + tea.StringValue(ak) + ",Signature:" + tea.StringValue(sign))
        return _result, _err
      }

      _result = tea.String("OSS2 AccessKeyId:" + tea.StringValue(ak) + ",AdditionalHeaders:" + tea.StringValue(additionalHeaders) + ",Signature:" + tea.StringValue(sign))
      return _result, _err
    }
...
func V2UriEncode (value *string) (_result *string) {
  if tea.BoolValue(util.Empty(value)) {
    _result = tea.String("")
    return _result
  }

  encoded := encodeutil.PercentEncode(value)
  encoded = string_.Replace(encoded, tea.String("+"), tea.String("%20"), nil)
  encoded = string_.Replace(encoded, tea.String("%7E"), tea.String("~"), nil)
  encoded = string_.Replace(encoded, tea.String("%2D"), tea.String("-"), nil)
  encoded = string_.Replace(encoded, tea.String("%5F"), tea.String("_"), nil)
  encoded = string_.Replace(encoded, tea.String("%2E"), tea.String("."), nil)
  _result = encoded
  return _result
}

func (client *Client) GetHeaderValue (headers map[string]*string, name *string) (_result *string, _err error) {
  if !tea.BoolValue(util.IsUnset(headers[tea.StringValue(name)])) {
    _result = headers[tea.StringValue(name)]
    return _result, _err
  }

  for _, header := range map_.KeySet(headers) {
    if tea.BoolValue(string_.Equals(string_.ToLower(header), name)) {
      _result = headers[tea.StringValue(header)]
      return _result, _err
    }
...
func (client *Client) GetAdditionalHeaderNamesV2 (headers map[string]*string) (_result []*string, _err error) {
  additionalHeaders := make(map[string]*string)
  for _, header := range map_.KeySet(headers) {
    lowerHeader := string_.ToLower(header)
    if tea.BoolValue(array.Contains(client.Default_additional_headers, lowerHeader)) {
      additionalHeaders[tea.StringValue(lowerHeader)] = lowerHeader
    }

  }
  _result = make([]*string, 0)
  _body := array.AscSort(map_.KeySet(additionalHeaders))
  _result = _body
  return _result, _err
}

func (client *Client) JoinSemicolon (items []*string) (_result *string, _err error) {
  result := tea.String("")
  separator := tea.String("")
  for _, item := range items {
    result = tea.String(tea.StringValue(result) + tea.StringValue(separator) + tea.StringValue(item))
    separator = tea.String(";")
  }
  _result = result
  return _result , _err
...
func (client *Client) BuildCanonicalizedOssHeadersV2 (headers map[string]*string, additionalHeaderNames []*string) (_result *string, _err error) {
  canonHeaders := make(map[string]*string)
  for _, header := range map_.KeySet(headers) {
    lowerHeader := string_.ToLower(header)
    if tea.BoolValue(string_.HasPrefix(lowerHeader, tea.String("x-oss-"))) {
      canonHeaders[tea.StringValue(lowerHeader)] = headers[tea.StringValue(header)]
    }

  }
  for _, name := range additionalHeaderNames {
    canonHeaders[tea.StringValue(name)], _err = client.GetHeaderValue(headers, name)
    if _err != nil {
      return _result, _err
    }
  ...
  canonicalizedHeaders := tea.String("")
  for _, header := range array.AscSort(map_.KeySet(canonHeaders)) {
    canonicalizedHeaders = tea.String(tea.StringValue(canonicalizedHeaders) + tea.StringValue(header) + ":" + tea.StringValue(canonHeaders[tea.StringValue(header)]) + "\n")
  }
  _result = canonicalizedHeaders
  return _result , _err
}

func (client *Client) BuildCanonicalizedQueryStringV2 (query map[string]*string) (_result *string, _err error) {
  queryMap := make(map[string]*string)
  if !tea.BoolValue(util.IsUnset(query)) {
    for _, queryKey := range map_.KeySet(query) {
      encodedKey := V2UriEncode(queryKey)
      encodedValue := tea.String("")
      if !tea.BoolValue(util.Empty(query[tea.StringValue(queryKey)])) {
        encodedValue = V2UriEncode(query[tea.StringValue(queryKey)])
      }
  ...
  if tea.BoolValue(util.IsUnset(queryMap)) || tea.BoolValue(util.EqualNumber(array.Size(map_.KeySet(queryMap)), tea.Int(0))) {
    _result = tea.String("")
    return _result, _err
  }
  ...
  canonicalizedQueryString := tea.String("?")
  separator := tea.String("")
  for _, key := range array.AscSort(map_.KeySet(queryMap)) {
    canonicalizedQueryString = tea.String(tea.StringValue(canonicalizedQueryString) + tea.StringValue(separator) + tea.StringValue(key))
    if !tea.BoolValue(util.Empty(queryMap[tea.StringValue(key)])) {
      canonicalizedQueryString = tea.String(tea.StringValue(canonicalizedQueryString) + "=" + tea.StringValue(queryMap[tea.StringValue(key)]))
    }
...
func (client *Client) BuildCanonicalizedResourceV2 (bucketName *string, pathname *string, query map[string]*string) (_result *string, _err error) {
  resourcePath := tea.String("/")
  if !tea.BoolValue(util.Empty(bucketName)) {
    resourcePath = tea.String("/" + tea.StringValue(bucketName) + tea.StringValue(pathname))
  } else if !tea.BoolValue(util.Empty(pathname)) {
...
func (client *Client) GetSignatureV2 (bucketName *string, pathname *string, method *string, query map[string]*string, headers map[string]*string, secret *string, additionalHeaderNames []*string) (_result *string, _err error) {
  contentMd5 := util.DefaultString(headers["content-md5"], tea.String(""))
  contentType := util.DefaultString(headers["content-type"], tea.String(""))
  date := util.DefaultString(headers["date"], tea.String(""))
  canonicalizedOssHeaders, _err := client.BuildCanonicalizedOssHeadersV2(headers, additionalHeaderNames)
  if _err != nil {
    return _result, _err
  }
  ...
  canonicalizedResource, _err := client.BuildCanonicalizedResourceV2(bucketName, pathname, query)
  if _err != nil {
    return _result, _err
  }

  stringToSign := tea.String(tea.StringValue(method) + "\n" + tea.StringValue(contentMd5) + "\n" + tea.StringValue(contentType) + "\n" + tea.StringValue(date) + "\n" + tea.StringValue(canonicalizedOssHeaders) + tea.StringValue(additionalHeaders) + "\n" + tea.StringValue(canonicalizedResource))
