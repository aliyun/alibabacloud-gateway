...
type Client struct {
	Default_signed_params []*string `json:"default_signed_params"`
	Except_signed_params  []*string `json:"except_signed_params"`
	spi.Client
}
...
func (client *Client) Init() (_err error) {
	...
	client.Except_signed_params = []*string{tea.String("list-type")}
	return nil
}
...
func (client *Client) ModifyRequest(context *spi.InterceptorContext, attributeMap *spi.AttributeMap) (_err error) {
	...
	res, err := client.GetHost(config.EndpointType, bucketName, config.Endpoint)
	if err != nil {
		return
	}
	...
	if !tea.BoolValue(util.IsUnset(response.Body)) {
		if tea.BoolValue(string_.Equals(request.BodyType, tea.String("xml"))) {
			...
			if tea.BoolValue(util.EqualNumber(array.Size(list), tea.Int(1))) {
				tmp := list[0]
				response.DeserializedBody = result[tea.StringValue(tmp)]
			} else {
...
func (client *Client) GetAuthorization(signatureVersion *string, bucketName *string, pathname *string, method *string, query map[string]*string, headers map[string]*string, ak *string, secret *string) (_result *string, _err error) {
	res1, err := client.GetSignatureV1(bucketName, pathname, method, query, headers, secret)
	if err != nil {
		return
	}
	res2, err := client.GetSignatureV2(bucketName, pathname, method, query, headers, secret)
	if err != nil {
		return
	}
	if tea.BoolValue(util.IsUnset(signatureVersion)) || tea.BoolValue(string_.Equals(signatureVersion, tea.String("v1"))) {
		_result = tea.String("OSS " + tea.StringValue(ak) + ":" + tea.StringValue(res1))
		return _result, _err
	} else {
		_result = tea.String("OSS2 AccessKeyId:" + tea.StringValue(ak) + ",Signature:" + tea.StringValue(res2))
		return _result, _err
	}
...
func (client *Client) BuildCanonicalizedResource(pathname *string, query map[string]*string) (_result *string, _err error) {
	...
	for k := range subResourcesMap {
		subResourcesArray = append(subResourcesArray, tea.String(k))
	}
